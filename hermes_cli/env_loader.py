...
def _apply_bitwarden_source(bw_cfg: dict, home_path: Path) -> None:
    """Pull secrets from Bitwarden into os.environ."""
    try:
...
def _apply_doppler_source(dop_cfg: dict, home_path: Path) -> None:
    ...
    result = apply_doppler_secrets(
        ...
        project=str(dop_cfg.get("project", "") or ""),
        config=str(dop_cfg.get("config", "") or ""),
        override_existing=bool(dop_cfg.get("override_existing", True)),
