...
_MEMORY_REVIEW_PROMPT = (
    ...
    "Before any memory(action=add/replace), apply Barry's built-in memory gate: "
    "prefer compact hard rules or durable user preferences near ~200 chars, but "
    "treat that as a soft warning target when durable Why/How-to-apply context is valuable. "
    "Only save facts that will still be useful in 30+ days. Do NOT save operational logs, PR/CI/"
    "commit/task status, incident narratives, transient topology, or debug traces; "
    "route those to Hindsight, session_search, Barry Memory RAG, plans, or skills.\n\n"
    "Focus on:\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "preferences with the memory tool only if they pass Barry's built-in "
    "memory gate: ~200 chars is a soft warning target, useful in 30+ days is mandatory, "
    "and operational logs/PR/CI/commit/task status/incident narratives/debug traces are rejected. Route "
    "anything else to Hindsight, session_search, Barry Memory RAG, plans, or skills.\n\n"
